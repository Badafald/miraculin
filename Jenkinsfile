                ...
                stage('Storage Service') {
                    steps {
                        script {
                            ...
                            def storExists = sh(script: "docker manifest inspect ${STOR_IMAGE} > /dev/null 2>&1 && echo 0 || echo 1", returnStdout: true).trim()
                ...
                stage('Web Interface Service') {
                    steps {
                        script {
                            ...
                            def webExists = sh(script: "docker manifest inspect ${WEB_IMAGE} > /dev/null 2>&1 && echo 0 || echo 1", returnStdout: true).trim()
